    ...
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-5 text-center">
        <div className="relative">
          <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-white/[0.06] flex items-center justify-center">
            <Wand2 className="w-7 h-7 text-zinc-700" />
          </div>
          <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-violet-600/20 border border-violet-500/30 flex items-center justify-center">
            <Sparkles className="w-2.5 h-2.5 text-violet-400" />
          </div>
        </div>
        <div>
          <p className="text-base font-semibold text-zinc-400">No images yet</p>
        ...
        <div className="flex flex-col gap-1.5 text-left">
          {["Add a clear prompt describing your scene", "Select brand references to anchor the style", "Choose an aspect ratio for your output"].map((tip) => (
            <div key={tip} className="flex items-center gap-2 text-xs text-zinc-700">
